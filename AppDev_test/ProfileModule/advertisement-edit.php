...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($_FILES['new_image']['error'] !== 4) {
        ...
        if (in_array($imageExtension, $validImageExtension) && $fileSize <= 1000000) {
            ...
            mysqli_query($conn, "UPDATE business_advertisement SET image = '$newImageName' WHERE id = $id");
        } else {
    ...
    mysqli_query($conn, "UPDATE business_advertisement SET name = '$newName', text = '$newText' WHERE id = $id");
